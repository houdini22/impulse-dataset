...
namespace Impulse {
    ...
    namespace Dataset {
        ...
        void DatasetSample::out() {
            for (T_Size i = 0; i < this->getSize() - 1; i++) {
                std::cout << this->getColumnToString(i) << ',';
            }
            std::cout << this->getColumnToString(this->getSize() - 1) << std::endl;
        }
        ...
        void DatasetSample::changeOrdering(std::initializer_list<int> columns) {
            T_StringVector result;
            result.reserve(columns.size());

            for (auto column : columns) {
                result.push_back(this->getColumnToString(column));
            }
